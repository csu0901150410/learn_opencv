...
typedef std::string (*_register_func_ptr)(const std::string&);// 被注册的函数指针类型
...
public:
    ...
    bool register_function(const std::string& name, _register_func_ptr func);

    _register_func_ptr get_function(const std::string& name);

private:
    Register() = default;
    std::unordered_map<std::string, _register_func_ptr> _registered_func;
};
